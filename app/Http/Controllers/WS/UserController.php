...
class UserController extends Controller
{
    ...
    public function profile_post(Request $request)
    {
        ...
        $user->extrovert = (int)Str::after($request['extrovert'], '%');
        $user->feeling = (int)Str::after($request['feeling'], '%');
        $user->intuition = (int)Str::after($request['intuition'], '%');
        $user->perceiving = (int)Str::after($request['perceiving'], '%');
        $user->maker = (int)Str::after($request['maker'], '%');
        $user->connector = (int)Str::after($request['connector'], '%');
        $user->idea_generator = (int)Str::after($request['idea_generator'], '%');
        $user->collaborator = (int)Str::after($request['collaborator'], '%');
        $user->finisher = (int)Str::after($request['finisher'], '%');
        $user->evaluator = (int)Str::after($request['evaluator'], '%');
        $user->organiser = (int)Str::after($request['organiser'], '%');
        $user->moderator = (int)Str::after($request['moderator'], '%');
        $user->save();
